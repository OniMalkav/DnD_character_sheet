      ...
      <Card className="shadow-xl">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="bg-muted/30 p-4 rounded-lg border border-border/50 flex flex-wrap items-end gap-6 overflow-x-auto min-h-[90px]">
            ...
            <div className="flex-1 min-w-[60px]">
              <div className="h-5 flex items-center justify-center">
                <Label className="text-xs font-bold uppercase tracking-widest text-center" style={{ color: THEME.colors.labels }}>Inspir</Label>
              </div>
            ...
            <div className="flex-[1.5] min-w-[120px]">
              ...
              <textarea
                ...
                className="mt-2 min-h-[80px] w-full rounded-md border border-input bg-black px-3 py-2 text-sm font-bold tracking-tight text-white placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* LEFT COLUMN: CLASS & SUBCLASS, MULTICLASS, RACE & BACKGROUND */}
            <div className="space-y-4">
              <div>
                <Label htmlFor="class1" className="text-xs font-bold uppercase tracking-widest block text-center" style={{ color: THEME.colors.labels }}>Class & Subclass</Label>
                <div className="flex gap-2 mt-1">
                  <Input id="class1" type="text" value={charInfo.class} onChange={(e) => updateCharInfo('class', e.target.value)} placeholder="e.g. Barbarian" className="flex-1" />
                  <Input type="text" value={charInfo.subclass} onChange={(e) => updateCharInfo('subclass', e.target.value)} placeholder="Subclass" className="flex-1" />
                  <Input type="number" value={charInfo.level} onChange={(e) => updateCharInfo('level', e.target.value)} placeholder="Lvl" className="w-16 text-center font-bold tracking-tight text-foreground/90" />
                </div>
              </div>
              <div>
                <Label htmlFor="class2" className="text-xs font-bold uppercase tracking-widest block text-center" style={{ color: THEME.colors.labels }}>Multiclass & Subclass</Label>
                <div className="flex gap-2 mt-1">
                  <Input id="class2" type="text" value={charInfo.class2} onChange={(e) => updateCharInfo('class2', e.target.value)} placeholder="e.g. Fighter" className="flex-1" />
                  <Input type="text" value={charInfo.subclass2} onChange={(e) => updateCharInfo('subclass2', e.target.value)} placeholder="Subclass" className="flex-1" />
                  <Input type="number" value={charInfo.level2} onChange={(e) => updateCharInfo('level2', e.target.value)} placeholder="Lvl" className="w-16 text-center font-bold tracking-tight text-foreground/90" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <Label htmlFor="background" className="text-xs font-bold uppercase tracking-widest block text-center" style={{ color: THEME.colors.labels }}>Background</Label>
            ...
            {/* RIGHT COLUMN: RESOURCE / ABILITY COUNTERS */}
            <div className="space-y-3 bg-muted/20 p-4 rounded-lg border border-border/50 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <Label className="text-xs font-bold uppercase tracking-widest flex items-center gap-2" style={{ color: THEME.colors.labels }}>
                    <Flame className="w-3.5 h-3.5" style={{ color: THEME.colors.primary }} /> Resource Counters
                  </Label>
                  <Button onClick={() => addCounter()} size="sm" variant="outline" className="h-7 text-[10px] uppercase font-bold tracking-widest">
                ...
                <div className="space-y-2 overflow-y-auto max-h-[175px] pr-1">
                  {counters && counters.length > 0 ? (
                    counters.map((counter) => (
                      <div key={counter.id} className="flex items-center gap-2 bg-background/70 p-2 rounded-md border border-border/60 hover:border-primary/40 transition-all">
                        <Input
                          type="text"
                          value={counter.name}
                          onChange={(e) => updateCounter(counter.id, 'name', e.target.value)}
                          placeholder="e.g. Rage, Second Wind"
                        ...
                        <div className="flex items-center bg-background rounded border h-8 px-1">
                          <Button
                            size="icon"
                            variant="ghost"
                            className="w-5 h-6 text-xs text-muted-foreground hover:text-foreground"
                            onClick={() => updateCounter(counter.id, 'current', Math.max(0, (parseInt(counter.current?.toString() || '0') || 0) - 1))}
                          >
                            -
                          </Button>
                          <Input
                            ...
                            onChange={(e) => updateCounter(counter.id, 'current', e.target.value)}
                            placeholder="0"
                            className="w-10 h-7 border-0 bg-transparent text-center text-xs font-bold p-0 focus-visible:ring-0"
                          />
                          <Button
                            size="icon"
                            variant="ghost"
                            className="w-5 h-6 text-xs text-muted-foreground hover:text-foreground"
                            onClick={() => updateCounter(counter.id, 'current', (parseInt(counter.current?.toString() || '0') || 0) + 1)}
                          >
                            +
                          </Button>
                        </div>
                        <span className="text-sm font-bold text-muted-foreground">/</span>
                        <div className="w-12 h-8 bg-background rounded border flex items-center overflow-hidden">
                          <Input
                            type="number"
                            value={counter.max}
                            onChange={(e) => updateCounter(counter.id, 'max', e.target.value)}
                            placeholder="Max"
                            className="w-full h-full border-0 bg-transparent text-center text-xs font-bold p-0 focus-visible:ring-0"
                          />
                        </div>
                        <Button
                          size="icon"
                    ...
                    <div className="text-center py-6 text-xs italic text-muted-foreground border border-dashed rounded-md border-border/40">
                      No counters added. Click <span className="font-bold text-foreground">+ Add Counter</span> to track uses (e.g. Rage, Second Wind).
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="sm:col-span-2 lg:col-span-3 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
              {featsList && featsList.length > 0 ? featsList.map((feat, index) => {
                ...
                return (
                  <div key={feat.id + '-' + index} className="bg-background/40 border border-border/50 rounded-lg overflow-hidden transition-all hover:border-border group" style={{ borderLeftWidth: '4px', borderLeftColor: typeColor }}>
                    <div
                      ...
                      onClick={() => setExpandedFeatIndex(isExpanded ? null : index)}
                    >
                      ...
                      <div onClick={(e) => e.stopPropagation()} className="shrink-0 mr-2">
                        <Select value={currentType} onValueChange={(val: any) => updateFeat(feat.id, 'type', val)}>
                          <SelectTrigger className="h-7 text-[10px] font-bold uppercase tracking-widest bg-transparent border-border/50 focus-visible:ring-1 px-2 w-[80px]">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
