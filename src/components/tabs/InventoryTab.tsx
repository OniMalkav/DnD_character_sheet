...
export default function InventoryTab() {
  ...
  const toggleDesc = (id: number) => setOpenDesc(prev => ({ ...prev, [id]: !prev[id] }));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in-0 slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-2">
                ...
                {consumables.map(item => (
                  <div key={item.id} className="space-y-1">
                    <div className="inventory-item-row gap-1">
                      <Input 
                        type="text" 
                        value={item.name}
                      ...
                      <Button 
                        size="icon" 
                        variant="ghost" 
                        className={cn("w-7 h-7 transition-colors", (item.description || openDesc[item.id]) ? "text-primary" : "text-muted-foreground")}
                    ...
                    {(openDesc[item.id] || (item.description && item.description.length > 0)) && (
                      <div className="pl-2 pr-1 pb-1">
                        <Input 
                          ...
                          onChange={(e) => updateConsumable(item.id, 'description', e.target.value)} 
                          placeholder="Item description or notes..." 
                          className="h-7 text-xs bg-background/40 border-primary/20 focus-visible:ring-1 italic"
                        />
                      </div>
                    )}
                  </div>
        ...
        <Card className="flex flex-col">
          <CardHeader className="space-y-1 pb-3">
            ...
            <div className="text-center">
              <span className="text-[10px] uppercase font-bold tracking-wider" style={{ color: THEME.colors.textMuted }}>Wearing / Type / AC • Hit • Dmg / Lbs</span>
            </div>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col">
            <ScrollArea className="flex-1 h-64 pr-3">
                        ...
                        <div className="flex items-center gap-1">
                          <Select 
                            ...
                            onValueChange={(val) => {
                              ...
                              // WEAPON AUTO-CALC LOGIC: Default Hit = Mod + PB, Default Damage Bonus = Mod
                              if (['STR', 'DEX', 'CON', 'INT', 'WIS', 'CHA'].includes(val)) {
                                ...
                                updateEquipmentItem(item.id, 'damageBonus', mod);
                          ...
                          {isWeapon && (
                            <>
                              <div className="flex items-center bg-background rounded border h-8 w-16 overflow-hidden">
                                <Input 
                                  type="text"
                                  value={item.damageDice ?? ''}
                                  onChange={(e) => updateEquipmentItem(item.id, 'damageDice', e.target.value)}
                                  className="w-full h-full border-0 bg-transparent text-center text-[10px] font-mono px-1 focus-visible:ring-0"
                                  placeholder="Dmg"
                                  title="Damage Dice (e.g. 1d8, 2d6)"
                                />
                              </div>
                              <div className="flex items-center bg-background rounded border h-8 w-11 overflow-hidden">
                                <Input 
                                  ...
                                  onChange={(e) => updateEquipmentItem(item.id, 'damageBonus', e.target.value)}
                                  className="w-full h-full border-0 bg-transparent text-center text-xs font-mono px-1 focus-visible:ring-0"
                                  placeholder="Mod"
                                  title="Damage Bonus (e.g. 3)"
                                />
                              </div>
                            </>
                          )}
                ...
                <CardContent className="flex flex-col pt-0">
                  <ScrollArea className="h-64 pr-3">
                    <div className="space-y-2">
                      ...
                      {bagItems.map(item => (
                        <div key={item.id} className="space-y-1">
                          <div className="inventory-item-row gap-1" style={{ borderColor: 'rgba(129, 140, 248, 0.2)' }}>
                            <Input 
                              type="text" 
                              value={item.name}
                          ...
                          {(openDesc[item.id] || (item.description && item.description.length > 0)) && (
                            <div className="pl-2 pr-1 pb-1">
                              <Input 
                                type="text" 
                                value={item.description || ''} 
                                onChange={(e) => updateBagItem(item.id, 'description', e.target.value)} 
                                placeholder="Item description or notes..." 
                                className="h-7 text-xs bg-background/40 border-primary/20 focus-visible:ring-1 italic"
                              />
                            </div>
                        ...
                        <div key={item.id} className="space-y-1">
                          <div className="inventory-item-row gap-1">
                            <Input 
                              type="text" 
                              value={item.name}
                              onChange={(e) => updateUntrackedItem(item.id, 'name', e.target.value)}
                              placeholder="Item name"
                              className="inventory-item-input"
                            />
                            <Button 
                          ...
                          {(openDesc[item.id] || (item.description && item.description.length > 0)) && (
                            <div className="pl-2 pr-1 pb-1">
                              <Input 
                                type="text" 
                                value={item.description || ''} 
                                onChange={(e) => updateUntrackedItem(item.id, 'description', e.target.value)} 
                                placeholder="Item description or notes..." 
                                className="h-7 text-xs bg-background/40 border-primary/20 focus-visible:ring-1 italic"
                              />
                            </div>
