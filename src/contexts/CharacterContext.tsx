...
import type { Stats, CharInfo, Currency, Consumable, EquipmentItem, InventoryItem, UntrackedItem, Spell, SpellSlots, Stat, DiceCounts, RollMode, SkillBonus, Feat, CustomCounter } from '@/lib/types';
...
type CharacterContextType = {
  ...
  updateUntrackedItem: (id: number, fieldOrName: keyof UntrackedItem | string, value?: any) => void;
  removeUntrackedItem: (id: number) => void;
  featsList: Feat[];
  addFeat: () => void;
  updateFeat: (id: number | string, field: keyof Feat, value: any) => void;
  removeFeat: (id: number | string) => void;
  moveFeat: (id: number | string, direction: 'up' | 'down') => void;
  counters: CustomCounter[];
  addCounter: (name?: string, current?: number | string, max?: number | string) => void;
  updateCounter: (id: number | string, field: keyof CustomCounter, value: any) => void;
  removeCounter: (id: number | string) => void;
  currency: Currency;
  ...
  toggleSpellPrepared: (id: number) => void;
...
export const CharacterProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const triggerRoll = (customCounts?: Partial<DiceCounts>, customMod?: number, customDmgMod?: number, label?: string, rollType?: 'check' | 'save' | 'attack') => {
    ...
    setTimeout(() => {
      ...
      let hitBonusTotal = 0;
      let dmgBonusTotal = 0;
      const hitBonusesList: { die: string, value: number }[] = [];
      const damageBonusesList: { die: string, value: number }[] = [];
      const baseDiceElementsList: { die: string, value: number }[] = [];
      ...
      if (isMultiAttack) {
        ...
        Object.entries(effectiveCounts).forEach(([die, count]) => {
          if (die !== 'd20' && count > 0) {
            ...
            baseDiceElementsList.push({ die, value: dieSum });
        ...
        Object.entries(hitBonuses).forEach(([die, active]) => {
          if (active) {
            ...
            if (!breakdown[die]) breakdown[die] = [];
            breakdown[die].push({ value: r });
            hitBonusesList.push({ die, value: r });
          }
        });
        ...
        Object.entries(damageBonuses).forEach(([die, active]) => {
          if (active) {
            ...
            if (!breakdown[die]) breakdown[die] = [];
            breakdown[die].push({ value: r });
            damageBonusesList.push({ die, value: r });
          }
        });
      ...
      const resultObj: RollResult = {
        ...
        hitBonusDice: !isMultiAttack ? hitBonusesList : undefined,
        damageBonusDice: !isMultiAttack ? damageBonusesList : undefined,
        baseDiceElements: !isMultiAttack ? baseDiceElementsList : undefined,
  ...
  const [charInfo, setCharInfo] = useState<CharInfo>({
    race: '', class: '', subclass: '', level: 1, class2: '', subclass2: '', level2: '',
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        if (data.characterName !== undefined) setCharacterName(data.characterName);
        if (data.charInfo) setCharInfo(prev => ({ ...prev, heroicInspiration: false, ...data.charInfo }));
        if (data.stats) setStats(data.stats);
        if (data.pb) setPb(data.pb);
        if (data.notes !== undefined) setNotes(data.notes);
        if (data.inventoryItems !== undefined) setInventoryItems(data.inventoryItems);
  ...
  useEffect(() => {
    ...
    const data = {
      characterName, charInfo, stats, pb, notes, inventoryItems, bagItems, consumables, equipmentItems, untrackedItems, featsList, counters, currency,
    ...
    equipmentItems, untrackedItems, featsList, counters, currency, spellAbility, spellSlots, spells, doubleCarry, profs, summonData
  ]);

  const updateStat = (stat: Stat, val: number) => setStats(prev => ({ ...prev, [stat]: val || 0 }));

  // COMPUTED EFFECTIVE STATS (Includes stat-setting magic items like Amulet of Health, Gauntlets of Ogre Power, etc.)
  const effectiveStats = React.useMemo(() => {
    const effective: Stats = { ...stats };
    const statKeys: (keyof Stats)[] = ['str', 'dex', 'con', 'int', 'wis', 'cha'];

    equipmentItems.forEach(item => {
      if (item.isWearing && item.armorType) {
        const key = item.armorType.toLowerCase() as keyof Stats;
        if (statKeys.includes(key)) {
          const val = parseInt(item.ac?.toString() || '0') || 0;
          if (val > 0) {
            effective[key] = Math.max(effective[key] || 0, val);
          }
        }
      }
    });
  ...
  useEffect(() => {
    ...
    if (armor) {
      // Treating the item's AC as a bonus/modifier rather than a base total
      const armorMod = parseInt(armor.ac?.toString() || '0') || 0;

      if (armor.armorType === 'Heavy') {
        baseAc = armorMod; // armor mod (No Dex for Heavy)
      } else if (armor.armorType === 'Medium') {
        baseAc = armorMod + Math.min(dexMod, 2); //armor mod + Max 2 Dex
      } else if (armor.armorType === 'Light') {
        baseAc = armorMod + dexMod; //armor mod + Full Dex
      }
    }
  ...
  }, [equipmentItems, effectiveStats.dex, isLoaded, charInfo.ac]);

  // AUTOMATED CONSTITUTION MODIFIER HP CALCULATION
  const prevConModRef = useRef<number | null>(null);

  useEffect(() => {
    if (!isLoaded) return;

    const conVal = effectiveStats.con;
    const currentConMod = Math.floor((conVal - 10) / 2);

    if (prevConModRef.current !== null && prevConModRef.current !== currentConMod) {
      const diff = currentConMod - prevConModRef.current;
      const lvl1 = parseInt(charInfo.level?.toString() || '1') || 0;
      const lvl2 = parseInt(charInfo.level2?.toString() || '0') || 0;
      const totalLevel = Math.max(1, lvl1 + lvl2);
      const hpChange = diff * totalLevel;

      setCharInfo(prev => {
        const currentMax = parseInt(prev.maxHp?.toString() || '10') || 10;
        const currentHp = parseInt(prev.hp?.toString() || '10') || 10;
        const newMax = Math.max(1, currentMax + hpChange);
        const newHp = Math.max(0, Math.min(newMax, currentHp + hpChange));
    ...
    prevConModRef.current = currentConMod;
  }, [effectiveStats.con, charInfo.level, charInfo.level2, isLoaded]);
  ...
  const updateCharInfo = (field: keyof CharInfo, val: any) => {
    setCharInfo(prev => {
      ...
      if (field === 'level' || field === 'level2') {
        const newLvl1 = field === 'level' ? (parseInt(val?.toString() || '0') || 0) : (parseInt(prev.level?.toString() || '0') || 0);
        const newLvl2 = field === 'level2' ? (parseInt(val?.toString() || '0') || 0) : (parseInt(prev.level2?.toString() || '0') || 0);
        
        newState.hitDieCount = newLvl1 + newLvl2;
      }
  ...
  const toggleProficiency = (skillName: string) => {
    setProfs(prev => {
      ...
      if (skillName.startsWith('SAVE_')) {
        ...
        const expKey = `EXP_${skillName}`;
        const isProf = newProfs.has(skillName);
        const isExp = newProfs.has(expKey);
  ...
  const addInventoryItem = () => setInventoryItems(prev => [...prev, { id: Date.now(), name: '', weight: 0, description: '' }]);
  ...
  const addBagItem = () => setBagItems(prev => [...prev, { id: Date.now(), name: '', weight: 0, description: '' }]);
  ...
  const addConsumable = () => setConsumables(prev => [...prev, { id: Date.now(), name: '', count: 1, weight: 0, description: '' }]);
  ...
  const addEquipmentItem = () => setEquipmentItems(prev => [...prev, { id: Date.now(), name: '', weight: 0, isWearing: false, armorType: 'None', ac: '', description: '', damageDice: '', damageBonus: '' }]);
  ...
  const addUntrackedItem = () => setUntrackedItems(prev => [...prev, { id: Date.now(), name: '', description: '' }]);
  const updateUntrackedItem = (id: number, fieldOrName: keyof UntrackedItem | string, value?: any) => {
    setUntrackedItems(prev => prev.map(item => {
      if (item.id !== id) return item;
      if (typeof fieldOrName === 'string' && value === undefined) {
        return { ...item, name: fieldOrName };
      }
      return { ...item, [fieldOrName]: value };
    }));
  };
  const removeUntrackedItem = (id: number) => setUntrackedItems(prev => prev.filter(item => item.id !== id));

  const addFeat = () => setFeatsList(prev => [...prev, { id: Date.now() + Math.random(), name: '', description: '', type: 'Feat' }]);
  const updateFeat = (id: number | string, field: keyof Feat, value: any) => {
    setFeatsList(prev => prev.map(f => f.id === id ? { ...f, [field]: value } : f));
  };
  const removeFeat = (id: number | string) => setFeatsList(prev => prev.filter(f => f.id !== id));
  const moveFeat = (id: number | string, direction: 'up' | 'down') => {
    setFeatsList(prev => {
  ...
  const addCounter = (name = '', current: number | string = 1, max: number | string = 1) => {
    setCounters(prev => [...prev, { id: Date.now() + Math.random(), name, current, max }]);
  };
  const updateCounter = (id: number | string, field: keyof CustomCounter, value: any) => {
    setCounters(prev => prev.map(c => c.id === id ? { ...c, [field]: value } : c));
  };
  const removeCounter = (id: number | string) => setCounters(prev => prev.filter(c => c.id !== id));
  ...
  const addSpell = (name: string, level: number) => {
    if (!name.trim()) return;
    setSpells(prev => [...prev, { id: Date.now(), name, level, prepared: false }].sort((a, b) => a.level - b.level || a.name.localeCompare(b.name)));
  };
  const removeSpell = (id: number) => setSpells(prev => prev.filter(s => s.id !== id));
  const toggleSpellPrepared = (id: number) => setSpells(prev => prev.map(s => s.id === id ? { ...s, prepared: !s.prepared } : s));
  ...
  const value = {
    characterName, setCharacterName, stats: effectiveStats, updateStat, pb, setPb, profs, toggleProficiency,
    ...
    updateUntrackedItem, removeUntrackedItem, featsList, addFeat, updateFeat, removeFeat, moveFeat,
    counters, addCounter, updateCounter, removeCounter,
    currency, updateCurrency, spellAbility, setSpellAbility,
    spellSlots, updateSpellSlotMax, toggleSpellSlot, longRest, shortRest, spells, addSpell, removeSpell, toggleSpellPrepared,
